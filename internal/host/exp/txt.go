...
// chapterHeaderRe khớp dòng đầu tiêu đề Markdown có số chương.
// Hỗ trợ cả định dạng tiếng Việt (# Chương N / ## Chương 12 ...) và tiếng Trung cũ (# 第N章).
var chapterHeaderRe = regexp.MustCompile(`^#+\s+(?:[Cc]hương\s+\d+|第.+?章)`)
