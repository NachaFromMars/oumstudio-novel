...
func TestRenderEPUB_StructuralInvariants(t *testing.T) {
	...
	if !strings.Contains(ch1, "Chương 1 Đêm Mưa") {
...
func TestRenderEPUB_LayeredVolume(t *testing.T) {
	locs := map[int]chapterLocation{
		1: {VolumeIdx: 1, VolumeTitle: "Đại Vương", IsFirstOfVolume: true},
		2: {VolumeIdx: 1, VolumeTitle: "Đại Vương"},
	}
	data, err := renderEPUB(
		"X",
		[]int{1, 2},
	...
	ch1 := files["OEBPS/chapter001.xhtml"]
	if !strings.Contains(ch1, `class="volume-divider"`) || !strings.Contains(ch1, "Tập 1 Đại Vương") {
