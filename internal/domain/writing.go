...
// UnmarshalJSON khoan dung (OUMStudio-Novel): nhận 3 dạng đầu vào
//  1. object thật: {"deviation":"...","suggestion":"..."}
//  2. JSON-encoded string của object: "{\"deviation\":...}" (model hay trả dạng này khi failover)
//  3. văn xuôi thuần: "Mô tả sai lệch... Đề xuất..." → dồn vào Suggestion
// Mục đích: không bao giờ làm hỏng commit_chapter chỉ vì định dạng feedback — đây là metadata phụ.
func (f *OutlineFeedback) UnmarshalJSON(data []byte) error {
	trimmed := strings.TrimSpace(string(data))
	if trimmed == "" || trimmed == "null" {
		return nil
	}
	// Trường hợp 1: object thật.
	if strings.HasPrefix(trimmed, "{") {
		type rawFeedback OutlineFeedback
		var r rawFeedback
	...
	// Còn lại: giải mã như chuỗi JSON.
	var s string
	if err := json.Unmarshal(data, &s); err != nil {
		// Không phải chuỗi hợp lệ → bỏ qua, không làm hỏng commit.
		return nil
	}
	s = strings.TrimSpace(s)
	// Trường hợp 2: chuỗi đó lại là JSON-encoded object.
