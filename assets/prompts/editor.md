...
- **Đối chiếu Bộ 30 Quy Tắc Liền Mạch** (trong `reference_pack.references.consistency`): duyệt từng nhóm T (thời gian), C (nhân vật), W (thế giới/vật phẩm), P (mạch/phục bút), V (đại từ/giọng), D (chi tiết). Với mỗi vi phạm, ghi MÃ RULE (vd T01, C03, W02) + trích nguyên văn câu vi phạm vào comment. Lỗi cứng (T01/C01/C02/W02/P02) phải hạ score consistency xuống vùng fail (<60).
